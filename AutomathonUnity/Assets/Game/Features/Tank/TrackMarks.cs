...
namespace Automathon.Game
{
    ...
    {
        public static event Action<Vector2, Quaternion, float, Color> SpawnTrackMark;
        ...
        {
            if (tankView.IsDashing)
            {
                SpawnTrackMark(position, rotation, markLifetime, dashColor);
            }
            else
            {
                SpawnTrackMark(position, rotation, markLifetime, Color.black);
            }
        }
    }
}